...
export function Login() {
  ...
  if (!isGoogleAuthConfigured) {
    return (
      <div
        className={`bg-gradient-to-br from-coffee-800 to-coffee-900 flex items-center justify-center p-4 ${
          isMobileLandscape ? "min-h-[100dvh] overflow-y-auto" : "full-page"
        }`}
      >
        <div
          className={`w-full max-w-md text-center rounded-3xl bg-white/5 backdrop-blur-xl border border-white/10 login-card-enter ${
            isMobileLandscape ? "p-4" : "p-8"
          }`}
        >
          <div
            className={`flex justify-center ${
              isMobileLandscape ? "mb-3" : "mb-6"
            }`}
          >
          ...
          <h1
            className={`font-bold text-white ${
              isMobileLandscape ? "text-xl mb-2" : "text-2xl mb-3"
            }`}
          >
            Connect Locally
          </h1>
          <p
            className={`text-white/60 ${
              isMobileLandscape ? "mb-3 text-sm" : "mb-6"
            }`}
          >
          ...
          <a
            ...
            className={`inline-flex items-center gap-2 rounded-xl bg-accent text-white font-semibold hover:bg-accent-light transition-all duration-300 shadow-lg shadow-accent/30 ${
  ...
  return (
    <div
      className={`flex flex-col lg:flex-row ${
        isMobileLandscape ? "min-h-[100dvh] overflow-y-auto" : "min-h-[100dvh]"
      }`}
    >
