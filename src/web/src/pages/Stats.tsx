...
export function Stats() {
  ...
  const maxShots =
    weeklyData.length > 0 ? Math.max(...weeklyData.map((d) => d.shots), 1) : 5;

  // Format avg shot time from ms to seconds
  const avgShotTimeSec =
    stats.avgBrewTimeMs > 0 ? (stats.avgBrewTimeMs / 1000).toFixed(1) : null;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          subtext={
            stats.shotsSinceBackflush > 100 ? "Time to clean!" : "Looking good"
          }
        ...
        <MetricCard
          ...
          subtext={
            stats.minBrewTimeMs > 0
              ? `${(stats.minBrewTimeMs / 1000).toFixed(0)}-${(
                  stats.maxBrewTimeMs / 1000
                ).toFixed(0)}s range`
              : undefined
          }
          color="purple"
        ...
        <Card>
          ...
          <div className="space-y-4">
            ...
            <StatRow
              ...
              value={
                stats.minBrewTimeMs > 0
                  ? `${(stats.minBrewTimeMs / 1000).toFixed(1)}s`
                  : "—"
              }
            ...
            <StatRow
              label="Longest Shot"
              value={
                stats.maxBrewTimeMs > 0
                  ? `${(stats.maxBrewTimeMs / 1000).toFixed(1)}s`
                  : "—"
              }
        ...
        <Card>
          ...
          <div className="space-y-4">
            ...
            <StatRow
              label="Energy Used"
              value={
                stats.totalKwh > 0 ? `${stats.totalKwh.toFixed(1)} kWh` : "—"
              }
    ...
    >
      ...
      <div
        className={`text-2xl font-bold ${
          isOverdue ? "text-amber-500" : "text-theme"
        }`}
      >
        {shotsSince}
      </div>
